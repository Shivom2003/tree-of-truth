...
import { ArrowLeft, BookOpen, HelpCircle, ArrowRight, Sparkles, Quote } from "lucide-react";
...
export default function RootsPage() {
  ...
  // Case 1: Overview Dashboard (/roots)
  if (slugArr.length === 0) {
    const mainRoots = [
      { id: "vedanta", title: "Vedanta Wisdom", code: "R1", desc: "Advaita, Dvaita, Samkhya, Upanishads, and Gitas.", color: "from-amber-500/10 to-yellow-600/5", q: "Is the apparent separation of the self and cosmos the ultimate illusion?" },
      { id: "buddhism", title: "Buddhist Thought", code: "R2", desc: "Anatta (no-self), Sunyata (emptiness), Rigpa (pure awareness), and Zen.", color: "from-orange-500/10 to-amber-600/5", q: "If the self is an illusion, what is it that experiences the illusion?" },
      { id: "phenomenology", title: "Phenomenology", code: "R3", desc: "First-person study of consciousness, reduction, self models, and presence.", color: "from-yellow-600/10 to-yellow-700/5", q: "What is left when all assumptions about the external world are bracketed?" },
      { id: "esoteric", title: "Esoteric Realities", code: "R4", desc: "Rebirth, subtle body maps, astral travel, NDEs, and enlightenment.", color: "from-amber-600/10 to-yellow-800/5", q: "What do anomalous and mystical states suggest about the continuity of awareness?" },
      { id: "inquiry", title: "Direct Inquiry", code: "R5", desc: "Atma Vichara (self-inquiry), Neti Neti, and investigation of the Witness.", color: "from-yellow-500/10 to-amber-700/5", q: "Who is the one asking the question?" },
      { id: "shivom", title: "Shivom's Inquiry", code: "R6", desc: "Deep sleep witness, congenital sensory impairment, and memory loss arguments.", color: "from-amber-700/10 to-orange-800/5", q: "What remains when memories, senses, language, and the ego vanish completely?" }
    ];
    ...
    return (
      <div className="relative min-h-screen bg-cosmic-dark text-gold-light pb-24 font-sans">
        ...
        <section className="relative pt-32 pb-12 px-6 md:px-12 max-w-6xl mx-auto text-center overflow-hidden">
          ...
          <span className="fade-up-element text-xs uppercase tracking-widest text-gold-bright mb-2 block font-serif">NAVIGATING THE SOIL</span>
          ...
          <p className="fade-up-element max-w-2xl mx-auto text-sm md:text-base text-gold-matte/80 leading-relaxed font-light mb-12">
            The oldest and deepest knowledge. What the contemplative and philosophical traditions arrived at through direct, first-person investigation over millennia.
          </p>
        </section>
        ...
        <main className="px-6 md:px-12 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {mainRoots.map((root) => (
            <Link
              key={root.id}
              href={`/roots/${root.id}`}
              className={`fade-up-element group glass-panel glass-panel-hover p-6 rounded-2xl border border-gold-matte/15 bg-gradient-to-br ${root.color} flex flex-col justify-between h-72`}
            >
              <div>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <Sparkles className="w-4 h-4 text-gold-matte/40 group-hover:text-gold-bright group-hover:scale-110 transition-all" />
                </div>
              ...
              <span className="text-[10px] text-gold-bright font-semibold uppercase tracking-wider flex items-center gap-1 group-hover:translate-x-1 transition-all mt-4">
                Explore Root <ArrowRight className="w-3.5 h-3.5" />
              </span>
            </Link>
          ))}
        </main>
  ...
  const node = TREE_NODES.find(n => n.path === `/roots/${key}`);
  const nodeId = node?.id;

  const isSubroot = slugArr.length > 1;
  const parentId = slugArr[0];

  // Retrieve sibling subroots if we are on a parent page (to allow subroot navigation)
  const subrootKeys = Object.keys(ROOTS_DATA).filter(
    ...
    <div className="relative min-h-screen bg-cosmic-dark text-gold-light pb-24 font-sans">
      ...
      <section className="relative pt-32 pb-12 px-6 md:px-12 max-w-5xl mx-auto border-b border-gold-matte/10 overflow-hidden">
        ...
        <div className="fade-up-element flex flex-wrap items-center gap-2 text-[10px] md:text-xs uppercase tracking-widest text-gold-matte mb-4">
          ...
          <span>/</span>
          <Link href="/roots" className="hover:text-gold-bright transition-colors">
            ROOTS
          </Link>
          <span>/</span>
          {isSubroot ? (
            <>
              <Link href={`/roots/${parentId}`} className="hover:text-gold-bright transition-colors">
      ...
      <main className="px-6 md:px-12 max-w-5xl mx-auto mt-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 flex flex-col gap-12">
          ...
          {/* Subroots navigation block if on parent root page */}
          {!isSubroot && subrootKeys.length > 0 && (
            <div className="fade-up-element border-t border-b border-gold-matte/10 py-6 my-2">
              <h3 className="text-xs uppercase tracking-wider text-gold-bright font-bold mb-4 flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5" /> Diversified Subroots
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {subrootKeys.map((subKey) => {
                  const subData = ROOTS_DATA[subKey];
                  const subSlug = subKey.split("/")[1];
                  return (
                    <Link
                      key={subKey}
                      href={`/roots/${parentId}/${subSlug}`}
          ...
          <div className="fade-up-element glass-panel p-5 rounded-2xl border border-gold-matte/15">
            ...
            <div className="flex flex-col gap-2 text-xs">
              {data.thinkers.map((thinkerName, i) => {
                // Find thinker key matching the name
